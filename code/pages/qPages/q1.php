...
<body>
	<main role="main">
		<aside class="aside col-md-2">
			...
			<h3 class="text-c">简单的排序</h3>
			<p class="desturction">  右边给出了一个sortABC()函数,他的作用是对传进的三个数值（实际上是三个数值的地址，也就是指针）进行<strong>自小到大</strong>的排序。试着补全这个函数,完成上述功能。
			<div class="tips">
				<h4>小贴士:</h4>
				<ul>
					<!--这里是说明  修改这里！！-->
					<li><span class="label label-primary radius va-m">1</span><span>注意函数的传参方式是按指针专递，而不是按值传递</span></li>
					<li><span class="label label-primary radius">2</span><span>编辑器中给出的代码并不是全部的代码，你只需要按照题目要求完成对应的函数，我们会在后台测试你的代码是否可行</span></li>
					<li><span class="label label-primary radius">3</span><span>我们希望看到更‘优雅’的代码，而不是‘一坨一坨’的</span></li>
